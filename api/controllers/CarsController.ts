...
export default class CarsController {
    ...
    async getModels(brand: string): Promise<APIResponse> {
        return await this.request.get(`/api/cars/models/${brand}`);
    }   
    ...
    async deleteCar(carId: number, sid: string): Promise<APIResponse> {
        return await this.request.delete(`/api/cars/delete/${carId}`);
    }
    async getCarById(carId: number, sid: string): Promise<APIResponse> {
        return await this.request.get(`/api/cars/${carId}`, {
            headers: {
                'Cookie': sid
            }
        });
